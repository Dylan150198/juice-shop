...
exports.encrypt = function encrypt(text) {
  ...
  sign.write(String(text));
  sign.end();
  return signature = sign.sign(insecurity.privateKey, 'hex');
 }
 
 exports.decrypt = function decrypt(text) {
    const verify = crypto.createVerify('SHA256');
    verify.write(String(text));
    verify.end();
    return verify.verify(insecurity.publicKey, signature, 'hex');
 }
